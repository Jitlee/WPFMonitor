        ...
        private void PaintCalibration(Size size)
        {
            try
            {
                ...
                for (int i = 0; i <= mainScale; i++)
                {
                    ...
                    text.SetValue(Canvas.LeftProperty, width * Math.Sin(mainAngle) * 258d / 611d - textSize.Width / 2d + width * 0.5d);
                    text.SetValue(Canvas.TopProperty, width * Math.Cos(mainAngle) * 258d / 611d - textSize.Height / 2d + width * 0.5d);
