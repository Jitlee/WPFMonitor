       ...
       public DataTable GetDeviceHistoryReport(string devName, int nDeviceID, DateTime begin, DateTime end, int StationID)
        {
            ...
            for (t = t1; t <= t2; t++)
            {
                ...
                //            as 设备  as 测点   as 观测值  as 观测时间
                string strSql = string.Format(@"select t2.DeviceName,t3.ChannelName,
t1.MonitorValue,t1.MonitorTime from {0} t1,t_Device t2, t_Channel t3 
where t1.DeviceID=t2.DeviceID and t3.deviceid=t1.deviceid and t3.channelno = t1.channelno 
and t1.DeviceID={1} and t1.MonitorTime between '{2}' and '{3}' and t1.StationID='{4}'",
 tableName2, nDeviceID, time1, time2, StationID);
        ...
        public DataTable GetChannelHistorReport(string devName, int DeviceID, int ChannelID, DateTime begin, DateTime end, int stationID)
        {
            ...
            for (t = t1; t <= t2; t++)
            {
                ...
                // as 设备         as 测点  as 观测值   as 观测时间
                string strSql = string.Format(@"select t2.DeviceName,t3.ChannelName,t1.MonitorValue,
t1.MonitorTime from {0} t1,t_Device t2,t_Channel t3  
where t1.DeviceID=t2.DeviceID and t3.deviceid=t1.deviceid and t3.channelno = t1.channelno and t1.DeviceID={1} and t1.ChannelNo ={2} and t1.MonitorTime between '{3}' and '{4}' ", tableName2, DeviceID, ChannelID, time1, time2);
        ...
        {
            // as 报警内容       as 发生时间     as 设备   as 解除时间     as 报警级别     as 操作员ID    as 策略ID     as 机房名
            DataTable dt = new DataTable();
            string strSql = string.Format(@"select t1.Content,t1.HappenTime,t2.DeviceName,
t1.RelieveTime,t1.AlarmLevel,t1.OperateUserID,t1.PolicyID,t2.StationID 
